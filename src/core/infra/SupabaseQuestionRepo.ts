...
export const supabaseQuestionRepo: QuestionRepo = {
  async fetchQuestions(): Promise<Question[]> {
    const { data, error } = await supabase.from("questions").select("id, text, weight").order("id")
  ...
  async fetchAllOptions(): Promise<Option[]> {
    const { data, error } = await supabase.from("options").select("id, question_id, text, value, icon").order("id")
